...
export class FixNullLevelId1769370000000 implements MigrationInterface {
  ...
  public async up(queryRunner: QueryRunner): Promise<void> {
    ...
    await queryRunner.query(
      `UPDATE chapters SET level_id = 1 WHERE level_id IS NULL`,
    );

    // After fixing values, set NOT NULL constraint
    await queryRunner.query(
      `ALTER TABLE chapters ALTER COLUMN level_id SET NOT NULL`,
    );
  }
  ...
  public async down(queryRunner: QueryRunner): Promise<void> {
    ...
    await queryRunner.query(
      `ALTER TABLE chapters ALTER COLUMN level_id DROP NOT NULL`,
    );
  }
}
