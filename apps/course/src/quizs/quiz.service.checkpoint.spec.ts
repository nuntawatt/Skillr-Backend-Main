...
describe('QuizService (checkpoint)', () => {
  ...
  describe('findCheckpointsByLesson', () => {
    ...
    it('returns solution and score when attempted correct', async () => {
      lessonRepository.findOne!.mockResolvedValue({ lesson_id: 10, chapter_id: 7 } as any);
      checkpointRepository.find!.mockResolvedValue([
        {
          checkpointId: 1,
  ...
  describe('checkCheckpointAnswer', () => {
    ...
    it('keeps userXp xpEarned when answer is wrong', async () => {
      ...
      userXpRepository.findOne!.mockResolvedValue({ userId: 'u1', chapterId: 7, xpEarned: 5, checkpointStatus: 'PENDING' } as any);
      userXpRepository.save!.mockImplementation(async (x) => x);
      ...
      const savedXp = (userXpRepository.save as jest.Mock).mock.calls[0][0];
      expect(savedXp).toMatchObject({ xpEarned: 5, checkpointStatus: 'PENDING' });
