...
export class ProgressValidationService {
  ...
  async validateLessonCompletion(
    ...
    const isAlreadyCompleted =
      await this.lessonProgressService.isLessonCompleted(userId, lessonId);
    ...
    if (!canAccess) {
      errors.push(
        'ไม่สามารถเข้าถึงบทเรียนนี้ได้ ต้องเรียนบทเรียนก่อนหน้าให้จบก่อน',
      );
    }
  ...
  async validateChapterProgress(
    ...
    try {
      ...
      const isValid =
        await this.chapterProgressService.validateProgressConstraints(
          userId,
          chapterId,
        );
      if (!isValid) {
      ...
      const summary =
        await this.chapterProgressService.getChapterProgressSummary(
          userId,
          chapterId,
        );
      ...
      if (Math.abs(summary.progressPercentage - calculatedProgress) > 1) {
        warnings.push(
          `ความคืบหน้าไม่สอดคล้องกัน: แสดง ${summary.progressPercentage}%, ควรเป็น ${calculatedProgress}%`,
        );
      }
    } catch (error) {
  ...
  async validateVideoProgress(
    ...
    const isCompleted = await this.lessonProgressService.isLessonCompleted(
      userId,
      lessonId,
    );
    if (isCompleted) {
  ...
  private async canAccessLesson(
    ...
    try {
      ...
      const lessonItem = summary.items.find(
  ...
  async preventDuplicateProgress(
    userId: number,
    lessonId: number,
  ): Promise<boolean> {
    const progress = await this.lessonProgressService.getLessonProgress(
      userId,
      lessonId,
    );
  ...
  async validateProgressIntegrity(
    ...
    try {
      ...
      for (const item of summary.items) {
        if (item.status === LessonProgressStatus.COMPLETED) {
          ...
          (item.status === LessonProgressStatus.CURRENT ||
            item.status === LessonProgressStatus.LOCKED)
        ) {
      ...
      if (foundIncompleteAfterCompleted) {
        warnings.push(
          'พบ Item ที่ยังไม่จบอยู่หลัง Item ที่จบแล้ว อาจมีปัญหาการเรียงลำดับ',
        );
      }

      // ตรวจสอบว่ามีมากกว่า 1 Item ที่เป็น current
      const currentItems = summary.items.filter(
        (item) => item.status === LessonProgressStatus.CURRENT,
      );
      if (currentItems.length > 1) {
        errors.push(
          `พบ Item ปัจจุบันมากกว่า 1 รายการ: ${currentItems.length} รายการ`,
        );
      }
    } catch (error) {
      errors.push(
        `ไม่สามารถตรวจสอบความสมบูรณ์ของความคืบหน้าได้: ${error.message}`,
      );
    }
