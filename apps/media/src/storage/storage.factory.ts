...
export class StorageFactory {
  constructor(private readonly minio: MinioStorageService, private readonly aws: AwsStorageService) {}
  ...
  video(): StorageProvider {
    const provider = (process.env.STORAGE_PROVIDER_VIDEO ?? process.env.STORAGE_PROVIDER ?? 'minio').toLowerCase();
    if (provider === 's3' || provider === 'aws') return this.aws;
    return this.minio;
  }
}
