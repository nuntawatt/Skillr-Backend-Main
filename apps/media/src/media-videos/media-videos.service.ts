...
export class MediaVideosService {
  ...
  async triggerTranscode(videoId: string) {
    ...
    if (!role)
      throw new BadRequestException('MEDIACONVERT_ROLE_ARN not configured');

    const job = await this.aws.createMediaConvertJob(
      inputS3,
      outputPrefix,
      role,
    );
  ...
  async uploadVideoFileAndPersist(
    ...
    const maxSize = Number(
      process.env.VIDEO_MAX_SIZE_BYTES ?? String(500 * 1024 * 1024),
    ); // 500MB for form upload
    ...
    const safeName = this.sanitizeFilename(
      file.originalname || `${videoId}.mp4`,
    );
    const key = `videos/${videoId}/${safeName}`;

    // Upload to storage
    await storage.putObject(bucket, key, file.buffer, file.size, {
      'Content-Type': file.mimetype,
    });

    const ownerUserId = Number(ownerUserIdFromBody ?? 0);
    const publicUrl =
      typeof storage.buildPublicUrl === 'function'
        ? storage.buildPublicUrl(bucket, key)
        : undefined;
  ...
  async createPresignedUpload(
    ...
    const maxSize = Number(
      process.env.VIDEO_MAX_SIZE_BYTES ?? 2 * 1024 * 1024 * 1024,
    ); // 2GB
    ...
    const videoId = randomUUID();
    const safeName = this.sanitizeFilename(
      dto.original_filename ?? `${videoId}.mp4`,
    );
    const key = `videos/${videoId}/${safeName}`;
  ...
  async getPresignedViewUrl(id: number) {
    ...
    const presignGet =
      (storage as any).presignGet ?? (storage as any).presignedGetObject;
    if (!presignGet || typeof presignGet !== 'function') {
      throw new BadRequestException(
        'presign GET not supported by storage provider',
      );
    }

    const expiresSeconds = Number(process.env.PRESIGN_EXPIRES_SECONDS ?? 3600); // 1 hour default
    const presignedUrl = await presignGet.call(
      storage,
      bucket,
      key,
      expiresSeconds,
